...
(function () {
  ...
  const makeLayoutSignature = (store = {}) => {
    ...
    let hash = 2166136261;
    items.forEach((item) => {
      const part = `${item?.id || ''}:${item?.type || ''}:${item?.parentId || ROOT_ID}`;
  ...
  const settleLayout = (store, layout, options = {}) => {
    ...
    const folderCount = Object.keys(layout.folders || {}).length;
    const isLargeLayout = fileCount > 900 || fileCount + folderCount > 1200;

    // Large maps need a different schedule: one compact folder pass, one dot
    // placement pass, and one containment pass. Repeating the full solver on
    // thousands of dots gives little visual improvement but blocks animation and
    // dragging. Small maps keep the richer multi-stage polish below.
    if (isLargeLayout) {
      updateFreeFolderSizes(store, layout, { lockedIds, force: true });
      forceFolderWrapContainment(store, layout, { lockedIds });
      activeScopes.forEach((folderId) => {
        const passes = getAdaptiveFolderPasses(store, folderId, 0.42);
        if (passes > 0) settleFolderRegionsInScope(store, layout, folderId, { lockedIds, passes });
      });
      updateFreeFolderSizes(store, layout, { lockedIds, force: true });
      forceFolderWrapContainment(store, layout, { lockedIds });
      activeScopes.forEach((folderId) => {
        const passes = getAdaptiveFilePasses(store, folderId, 0.72);
        if (passes > 0) settleFilesInFixedScope(store, layout, folderId, { lockedIds, passes });
      });
      updateFreeFolderSizes(store, layout, { lockedIds, force: true });
      forceFolderWrapContainment(store, layout, { lockedIds });
      growScopesForFileObstacleConflicts(store, layout, { lockedIds });
      forceFolderWrapContainment(store, layout, { lockedIds });
      activeScopes.forEach((folderId) => {
        const passes = getAdaptiveFilePasses(store, folderId, 0.36);
        if (passes > 0) settleFilesInFixedScope(store, layout, folderId, { lockedIds, passes });
      });
      updateFreeFolderSizes(store, layout, { lockedIds, force: true });
      forceFolderWrapContainment(store, layout, { lockedIds });
      getLayoutScopes(store).forEach((folderId) => clampDirectChildrenIntoFolder(store, layout, folderId, { lockedIds }));
      return;
    }
  ...
  const ensureLayout = (store = {}, options = {}) => {
    if (!options.__indexed) {
      const layout = store.graphLayout && typeof store.graphLayout === 'object' ? store.graphLayout : null;
      const lockedIds = options.lockedIds || new Set();
      const canUseValidatedLayout = options.settle === false
        && !options.forceSettle
        && (!lockedIds || !lockedIds.size)
        && Number(layout?.version || 0) === LAYOUT_VERSION
        && layout?.folders
        && layout?.files
        && layout?.settleMeta?.signature === makeLayoutSignature(store);
      if (canUseValidatedLayout) return layout;
  ...
  const createKnowledgeMapController = ({
    ...
    const workspaceMountParent = mapView?.parentNode || null;
    const workspaceMountNextSibling = mapView?.nextSibling || null;

    const selectedFileIds = new Set();
    const searchMatchedFileIds = new Set();
    let searchQuery = '';
    ...
    let activeFrameLayout = null;
    let cachedViewportSize = { width: 0, height: 0 };
    ...
    const mountIn = (container, mountName = 'assist') => {
      if (!mapView || !container || mapView.parentNode === container) return false;
      container.appendChild(mapView);
      cachedViewportSize = { width: 0, height: 0 };
      mapView.dataset.mount = mountName;
      scheduleRevealPrewarm();
      requestRender();
      return true;
    };

    const mountInWorkspace = () => {
      if (!mapView || !workspaceMountParent || mapView.parentNode === workspaceMountParent) return false;
      const anchor = workspaceMountNextSibling?.parentNode === workspaceMountParent
        ? workspaceMountNextSibling
        : null;
      workspaceMountParent.insertBefore(mapView, anchor);
      cachedViewportSize = { width: 0, height: 0 };
      mapView.dataset.mount = 'workspace';
      scheduleRevealPrewarm();
      requestRender();
      return true;
    };
    ...
    const getLayout = (options = {}) => {
      ...
      if (activeFrameLayout && settle === false && !lockedIds.size && !options.freezeFolders) {
        return activeFrameLayout;
      }
      return ensureLayout(getStoreSafe(), {
        settle,
        lockedIds,
        freezeFolders: Boolean(options.freezeFolders),
      });
    ...
    const getViewportSize = () => {
      if (cachedViewportSize.width > 1 && cachedViewportSize.height > 1) {
      ...
      if (measured.width > 1 && measured.height > 1) cachedViewportSize = measured;
      return measured;
    };
    ...
    const updateChrome = () => {
      ...
      const nextTitle = folder?.id === rootId ? '知识点图谱' : `知识点图谱 · ${folder?.name || '未命名文件夹'}`;
      if (titleEl && nextTitle !== lastChromeTitle) {
        titleEl.textContent = nextTitle;
        lastChromeTitle = nextTitle;
      }
      if (!hintEl) return;
      let nextHint = '左键拖动画布；右键框选文件点；文件点直接跟随光标；左键双击文件夹区域并按住第二次点击可拖动文件夹。';
      if (dragState?.type === 'folder') nextHint = '正在拖动文件夹区域：内部文件点和子文件夹会整体跟随。';
      else if (dragState?.type === 'files' && hoverFolderId) nextHint = hoverFolderId === rootId ? '松手后移动到根知识仓库。' : '松手后移动到该文件夹区域。';
      else if (armedFolderId && now() <= armedFolderUntil) nextHint = '文件夹已进入拖动准备：再次按住该文件夹区域即可拖动。';
      else if (selectedFileIds.size > 1) nextHint = `已选择 ${selectedFileIds.size} 个知识点，拖动任意一个会聚拢为点群。`;
      if (nextHint !== lastChromeHint) {
        hintEl.textContent = nextHint;
        lastChromeHint = nextHint;
      }
    };
    ...
    const applyWorldTransform = () => {
      ...
      const transform = `translate(${layout.viewport.x.toFixed(2)} ${layout.viewport.y.toFixed(2)}) scale(${layout.viewport.scale.toFixed(4)})`;
      if (transform !== lastWorldTransform) {
        worldLayer.setAttribute('transform', transform);
    ...
    const renderFolders = () => {
      ...
      const orderedFolders = foldersByDepth(store, false);
      const folderOrderSignature = orderedFolders.map((folder) => folder.id).join('|');
      const shouldReorderFolders = renderCache.folderOrderSignature !== folderOrderSignature;
      orderedFolders.forEach((folder) => {
        ...
        if (cached.g.parentNode !== folderLayer || shouldReorderFolders) folderLayer.appendChild(cached.g);
        ...
        if (pulse > 0.001 && !renderInfo.dominant) {
          ...
          g.style.removeProperty?.('filter') || (g.style.filter = '');
        }
      ...
      renderCache.folderOrderSignature = folderOrderSignature;
    };
    ...
    const createFileNode = (fileId) => {
      ...
      // A single circle carries both the visible dot and the invisible hit area
      // via transparent stroke. This keeps file points fully interactive while
      // cutting per-file SVG children from three nodes to one.
    ...
    const renderFiles = () => {
      ...
      visibleFiles.forEach((file) => {
        ...
        const isSelectedFile = selectedFileIds.has(file.id);
        const isSearchMatch = searchMatchedFileIds.has(file.id);
        const isDeletingFile = isDeletingVisualItem(file.id);
        const scale = isDraggingFile ? 1.22 : (1 + filePulse * 0.34);
        const transform = `translate(${Number(point.x + pressure.x).toFixed(2)} ${Number(point.y + pressure.y).toFixed(2)}) scale(${scale.toFixed(3)})`;
        const radius = isSelectedFile ? '4.7' : '4.2';
        const key = `${transform}|${filePulse.toFixed(3)}|${isSelectedFile ? 1 : 0}|${isSearchMatch ? 1 : 0}|${isDraggingFile ? 1 : 0}|${isDeletingFile ? 1 : 0}|${radius}|${file.name || ''}`;
        if (cached.lastKey !== key) {
          g.setAttribute('transform', transform);
          g.style.setProperty('--map-pulse', filePulse.toFixed(3));
          g.classList.toggle('is-selected', isSelectedFile);
          g.classList.toggle('is-search-match', isSearchMatch);
          g.classList.toggle('is-dragging', isDraggingFile);
          g.classList.toggle('is-deleting', isDeletingFile);
          dot.setAttribute('r', radius);
          g.setAttribute('aria-label', `${file.name || '未命名文件'}${isSearchMatch ? '，搜索匹配' : ''}`);
          cached.lastKey = key;
        }
      });
    ...
    const renderOverlay = () => {
      ...
      if (dragState && hoverFolderId && hoverFolderId !== rootId) {
        const layout = getLayout({ settle: false });
        dropTarget = getVisualFolderLayout(hoverFolderId, layout.folders?.[hoverFolderId]) || null;
      }
      const selectionKey = selectionState
        ? `${selectionState.start.x},${selectionState.start.y},${selectionState.current.x},${selectionState.current.y}`
        : '';
      const dropKey = dropTarget
        ? `${hoverFolderId}:${dropTarget.x},${dropTarget.y},${dropTarget.rx},${dropTarget.ry}`
        : '';
      const overlayKey = `${selectionKey}|${dropKey}`;
    ...
    const render = () => {
      ...
      if (mapView.getAttribute?.('aria-hidden') === 'true' && shell?.dataset?.knowledgeMap === 'hidden') {
        mapView.classList?.remove?.('is-settling');
        return;
      }
      const store = getStoreSafe();
      const layout = ensureLayout(store, { settle: false });
      activeFrameLayout = layout;
      try {
        withStoreIndex(store, () => {
          ...
          updateChrome();
          lastRenderAt = now();
          if (stillAnimating || visualState.animating) requestRender();
        });
      } finally {
        activeFrameLayout = null;
      }
    };
    ...
    const scheduleLayoutSave = (delay = 140) => {
      window.clearTimeout(layoutSaveTimer);
      layoutSaveTimer = window.setTimeout(saveLayoutOnly, Math.max(0, Number(delay) || 0));
    };

    const syncStore = (options = {}) => {
      const store = getStoreSafe();
      const lockedIds = options.lockedIds || new Set();
      const currentLayout = store.graphLayout && typeof store.graphLayout === 'object' ? store.graphLayout : null;
      const layoutIsCurrent = !lockedIds.size
        && Number(currentLayout?.version || 0) === LAYOUT_VERSION
        && currentLayout?.folders
        && currentLayout?.files
        && currentLayout?.settleMeta?.signature === makeLayoutSignature(store);
      const layout = layoutIsCurrent
        ? ensureLayout(store, { settle: false })
        : settleAndSave({ lockedIds });
    ...
    const setSearchMatches = (fileIds = [], options = {}) => {
      const store = getStoreSafe();
      const validFileIds = new Set(
        (Array.isArray(fileIds) ? fileIds : [])
          .map(String)
          .filter((id) => isFile(getItem(store, id)))
      );
      const newlyMatched = [...validFileIds].filter((id) => !searchMatchedFileIds.has(id));
      const changed = validFileIds.size !== searchMatchedFileIds.size
        || [...validFileIds].some((id) => !searchMatchedFileIds.has(id));
      searchMatchedFileIds.clear();
      validFileIds.forEach((id) => searchMatchedFileIds.add(id));
      searchQuery = String(options.query || '');
      newlyMatched.forEach((id) => pulseVisual('file', id, 1.35));
      if (newlyMatched.length) visualState.animating = true;
      if (mapView) {
        mapView.dataset.searchMatchCount = String(searchMatchedFileIds.size);
        mapView.dataset.searchActive = searchQuery ? 'true' : 'false';
      }
      if (shell) shell.dataset.knowledgeSearchMatches = String(searchMatchedFileIds.size);
      if (changed || newlyMatched.length) requestRender();
      return Array.from(searchMatchedFileIds);
    };
    ...
    const moveViewportTo = (target, options = {}) => {
      ...
      if (options.immediate) {
        ...
        if (options.persist !== false) saveLayoutOnly();
        render();
        return;
      }
      viewportAnimation = {
        from,
        to: normalizeViewport(nextViewport),
        startedAt: now(),
        duration: 520,
    ...
    function tickViewportAnimation() {
      ...
      if (t < 1) window.requestAnimationFrame?.(tickViewportAnimation);
        ...
        const shouldPersist = viewportAnimation.persist !== false;
        viewportAnimation = null;
        if (shouldPersist) saveLayoutOnly();
      }
    }

    function focusFolder(folderId = rootId, options = {}) {
      const layout = options.persist === false ? getLayout({ settle: false }) : settleAndSave();
      ...
      const width = Math.max(320, viewportSize.width || 900);
      const height = Math.max(240, viewportSize.height || 560);
    ...
    async function leaveForFileOpen(options = {}) {
      ...
      if (layoutSaveTimer) saveLayoutOnly();
    ...
    function attachEvents() {
      ...
      if (window.ResizeObserver && !viewportResizeObserver) {
        viewportResizeObserver = new ResizeObserver((entries) => {
          if (mapView?.getAttribute?.('aria-hidden') === 'true') return;
          const box = entries?.[0]?.contentRect;
          if (!box || box.width < 40 || box.height < 40) return;
          const nextSize = { width: Number(box.width), height: Number(box.height) };
          if (Math.abs(nextSize.width - cachedViewportSize.width) < 0.5
            && Math.abs(nextSize.height - cachedViewportSize.height) < 0.5) return;
          cachedViewportSize = nextSize;
          const layout = getLayout();
          if (layout.viewport.userControlled) requestRender();
          else focusFolder(getStoreSafe()?.currentFolderId || rootId, {
            immediate: true,
            force: true,
            persist: false,
          });
        ...
        window.addEventListener('resize', () => {
          ...
          const layout = getLayout();
          if (layout.viewport.userControlled) requestRender();
          else focusFolder(getStoreSafe()?.currentFolderId || rootId, { immediate: true, force: true, persist: false });
        });
      }
    ...
    return {
      ...
      mountIn,
      mountInWorkspace,
      getMount: () => mapView?.dataset?.mount || (mapView?.parentNode === workspaceMountParent ? 'workspace' : 'assist'),
      focusFolder,
      animateDeleteItems,
      leaveForFileOpen,
      ensureLayout: () => ensureLayout(getStoreSafe(), { settle: true }),
      debugSelectedFileIds: () => Array.from(selectedFileIds),
      debugSearchMatchIds: () => Array.from(searchMatchedFileIds),
